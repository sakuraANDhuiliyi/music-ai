<script setup>
import { defineComponent, ref, watch, provide, onBeforeUnmount, onMounted } from "vue";

import MusicList from "./list/index.vue";
import MusicControl from "./controls/index.vue";
import QueuePanel from "./list/components/queue-panel.vue";
...
onBeforeUnmount(() => {
  removeAudio();
  try {
    if (typeof window !== "undefined" && window.__museaiMusic) {
      delete window.__museaiMusic;
    }
  } catch {}
});
// 用户关闭浏览器前保存信息
...
onMounted(() => {
  try {
    if (typeof window !== "undefined") {
      window.__museaiMusic = {
        setters: musicSetters,
...
.music {
  ...
  &-box {
    ...
    /* 去掉 justify-content: space-between 以免拉伸 */
  }
  ...
  .music-control-wrapper {
    ...
    width: 100%;
    z-index: 10;
    background-color: var(--music-main-bg); /* 防止透明 */
    /* 可选：添加一点阴影区分 */
