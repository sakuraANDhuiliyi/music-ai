...
import { gsapTransLyric, gsapTransLyricLeftToRight } from "@/utils/transform.js";

import SpecialTitle from "./special-title.vue";
import { debounce } from "@/utils/tool.js";
import useSpecial from "./useSpecial.js";
...
const calcLyricDuration = () => {
  const timeList = getMusicInfo.value?.lyricTimeList || [];
  const currentTime = Number(timeList[getCurrentLyricIndex.value]);
  const nextTime = Number(timeList[getCurrentLyricIndex.value + 1]);
  ...
  const fromDuration = duration >= 1500 ? 1.2 : duration >= 1000 ? 0.8 : 0;
  const leaveDuration = duration > 1500 ? 0.6 : duration >= 1000 ? 0.4 : 0;
...
const animateSpecial = () => {
  ...
  const specialEl = document.querySelector(".special-lyric");
  if (!specialEl) return;
  gsapArr = [];
